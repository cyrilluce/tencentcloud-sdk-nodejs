...
const Client = tencentcloud.scf.v20180416.Client;
...
let client = new Client(cred, "ap-guangzhou", clientProfile);

// 实例化一个请求对象,并填充参数
let req = new models.UpdateFunctionConfigurationRequest();
// 传入参数
req.deserialize({
  FunctionName: "hello-world-prod-hello",
  Description: `example`
});

// 通过client对象调用想要访问的接口，需要传入请求对象以及响应回调函数
client.UpdateFunctionConfiguration(req, function(err, response) {
  // 请求异常返回，打印异常信息
  if (err) {
    console.log(err);
    return;
